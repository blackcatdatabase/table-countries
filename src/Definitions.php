...
final class Definitions {
    ...
    /** @return string[] */
    public static function columns(): array { return [ 'iso2', 'name', 'created_at', 'updated_at' ]; }
     ...
     * iso2 may be "id" or "col1, col2".
     * @return string[]
     */
    public static function pkColumns(): array {
        $raw = trim('iso2');
        if ($raw === '') {
            throw new \InvalidArgumentException('Definitions::pkColumns(): token iso2 must not be empty.');
        }
    ...
    public static function defaultOrder(): ?string {
        $c = trim('created_at DESC, iso2 DESC'); return $c !== '' ? $c : null;
    }
