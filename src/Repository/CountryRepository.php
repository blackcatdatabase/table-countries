...
namespace BlackCat\Database\Packages\Countries\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Countries\Definitions;
use BlackCat\Database\Packages\Countries\Criteria;
use BlackCat\Database\Packages\Countries\Dto\CountryDto as Dto;
use BlackCat\Database\Packages\Countries\Mapper\CountryDtoMapper as RowMapper;
...
final class CountryRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Countries\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'iso2' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Countries\Definitions::table());
            return $bulk->upsertMany($rows, [ 'iso2' ], [ 'name' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Countries\Dto\CountryDto|null */
    public function getByIso2(string $iso2, bool $asDto = false): array|\BlackCat\Database\Packages\Countries\Dto\CountryDto|null {
        $row = $this->getByUnique([ 'iso2' => $iso2 ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Countries\Mapper\CountryDtoMapper::fromRow($row);
    }
    public function existsByIso2(string $iso2): bool {
        $where = 't.' . Ident::q($this->db, 'iso2') . ' = :uniq_iso2';
        return $this->exists($where, [ 'uniq_iso2' => $iso2 ]);
    }
    /** @return int|string|null */
    public function getIdByIso2(string $iso2) {
        $row = $this->getByIso2($iso2);
        return $row ? ($row['iso2'] ?? null) : null;
    }
