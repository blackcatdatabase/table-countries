...
use BlackCat\Database\Packages\Countries\Repository\CountryRepositoryInterface;
...
class CountryRepository implements CountryRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'iso2' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Countries\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'name' ]);
          return count($rows);
        }
