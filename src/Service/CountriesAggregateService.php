 ...
 */
final class CountriesAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CountryRepository $countryRepo
    ) {
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\Countries\ModuleException("Record {$idStr} not found for locking.");
            }
